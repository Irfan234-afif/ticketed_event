  ...
  <div class="flex flex-col gap-12">
    ...
    <div class="flex flex-col gap-4">
      ...
      <p class="text-[14px] font-normal text-[#969696] leading-[20px] tracking-[-0.4px]">
        Please review your registration details for {{ store.event?.title || 'Lafiye Unveils' }}, including your
        selected arrival date and time. Kindly ensure all information is accurate before proceeding. Terms & Conditions
        apply.
      </p>
    </div>
    ...
    <div v-if="selectedScheduleDetails.length > 0" class="flex flex-col gap-4">
      ...
      <div class="flex flex-col gap-3">
        <div v-for="schedule in selectedScheduleDetails" :key="schedule.name"
...
function loadRecaptcha(siteKey: string) {
  ...
  if (window.grecaptcha && typeof window.grecaptcha.render === 'function') {
    // grecaptcha is ready, render immediately
  ...
  if (existingScript) {
    ...
    // Wait for it to be ready with polling
    waitForRecaptchaReady(siteKey)
    return
  }
  ...
  script.async = true
  script.defer = true
  script.onload = () => waitForRecaptchaReady(siteKey)
  script.onerror = () => {
...
function waitForRecaptchaReady(siteKey: string, attempts: number = 0) {
  ...
  if (window.grecaptcha && typeof window.grecaptcha.render === 'function') {
    ...
    setTimeout(() => waitForRecaptchaReady(siteKey, attempts + 1), 100)
  }
}
...
function renderWidget(siteKey: string) {
  ...
  if (!widgetElement) {
    console.error('reCAPTCHA widget element not found')
    return
  }
