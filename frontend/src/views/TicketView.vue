  ...
  <div class="max-w-lg mx-auto p-6 pb-16 min-h-screen bg-white flex flex-col" v-if="registration.data">
    ...
    <div class="mb-8 mt-4">
      ...
      <p class="text-[#969696] text-[14px] leading-[20px] mb-2">
        ...
        will be sent to your email. Please present it upon arrival.
      </p>
    ...
    <div class="mb-8">
      ...
      <div v-for="p in participants" :key="p.name" @click="openQR(p)"
    ...
    <div class="mb-4 mt-auto">
      <button @click="router.push('/')" class="w-full bg-black text-white py-4 rounded-full text-[16px] font-medium">
        Back to Home
      </button>
    </div>
  </div>
  ...
  <BottomSheet :show="showQR" @update:show="showQR = $event" @close="closeQR">
    ...
    <div class="p-6 flex flex-col items-center text-center">
      ...
      <div class="border rounded-xl p-4 shadow-sm mb-6 bg-white">
        <img v-if="selectedParticipant?.qr_code_id" :src="getQRUrl(selectedParticipant.qr_code_id)" alt="QR Code"
          class="w-48 h-48 block" />
      </div>

      <h3 class="font-bold text-[18px] mb-1">{{ selectedParticipant?.full_name }}</h3>
      <p class="text-[14px] text-gray-500 mb-8" v-if="schedules.length > 0">
...
const schedules = computed(() => registrationDetails.data?.schedules || [])

const scheduleTitles = computed(() => {
  if (!schedules.value || schedules.value.length === 0) return 'Event'
  return schedules.value.map((s: any) => s.title).filter(Boolean).join(', ')
})
...
function formatSchedules(schedulesList: any[]) {
  ...
  return schedulesList.map(s => {
    ...
    const date = s.date ? new Date(s.date).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }) : ''
